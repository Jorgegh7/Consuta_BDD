...
SELECT numfactura AS "Nº Factura",
       TO_CHAR(fecha, 'dd') || ' de ' || 
       TO_CHAR(fecha, 'Month') || ' ' ||
       TO_CHAR (fecha, 'yyyy') AS "Fecha Emisión",
       LPAD(rutcliente, 10, '0') AS "Rut Cliente",
       neto AS "Monto Neto",
       iva AS "Monto Iva",
       total AS "Total Factura",  
       CASE  
            WHEN total > 0 AND total <= 50000 THEN 'Bajo'
            WHEN total >= 50001 AND total <= 100000 THEN 'Medio'
...
SELECT
      ...
      nombre AS "Cliente",
      NVL2(telefono, TO_CHAR(telefono),'Sin Teléfono') AS "TELÉFONO",
      NVL2(codcomuna, TO_CHAR(codcomuna), 'Sin Comuna') AS "COMUNA",
      estado AS "Estado",
      CASE
          WHEN saldo/credito < 0.5 THEN 'Bueno' || ' (' || '$' || TO_CHAR(saldo, 'FM999G999G999G999') || ')'
          WHEN saldo/credito > 0.5 AND saldo/credito < 0.8 THEN 'Regular' || ' (' || '$' || TO_CHAR(saldo, 'FM999G999G999G999') || ')'
          WHEN saldo/credito > 0.8 THEN 'Crítico'
      END AS "Estado Crédito",   
      NVL2(mail,SUBSTR(mail, INSTR(mail, '@') + 1), 'Correo No Registrado') AS "Dominio Correo"
FROM cliente
...
SELECT
      ...
      NVL2(valorcompradolar, valorcompradolar || ' USD', 'Sin Registro') AS "Compra en USD",
      NVL2(valorcompradolar, '$' || TO_CHAR((valorcompradolar * &cambio_dolar), 'FM999G999G999G999') || ' PESOS', 'Sin Registro') AS "USD convertido", 
      totalstock AS "Stock",
      CASE
      ...
      END AS "Alerta Stock",
      CASE 
          WHEN totalstock > &umbral_alto THEN TO_CHAR(vunitario * 0.9, 'FM$999G999')
      ELSE
          'N/A'
      END AS "Precio Oferta"    
FROM producto
WHERE procedencia = 'I' AND descripcion LIKE '%ZAPATO%' 
